...
public final class RhinoBeetleBait {
    public static final String ID = "rhino_beetle";
    ...
    public static ItemStack create(Plugin plugin, int amount) {
        ...
        m.displayName(Component.text("Rhino Beetle", NamedTextColor.AQUA));
        m.lore(List.of(
                Component.text("A type of bait found when", NamedTextColor.GRAY),
                Component.text("scavenging around ferns", NamedTextColor.GRAY)
        ));
        m.addItemFlags(ItemFlag.HIDE_ATTRIBUTES, ItemFlag.HIDE_ENCHANTS);
        m.setItemModel(NamespacedKey.fromString("longhardfish:bait/rhino_beetle"));
        s.setItemMeta(m);
    ...
    public static void registerInto(Plugin plugin) {
        BaitRegistry.register(new BaitRegistry.BaitDefinition(
                ...
                java.util.List.of(),
                "Rhino Beetle",     // singular
                "Rhino Beetles"    // plural
        ));
    }
}
